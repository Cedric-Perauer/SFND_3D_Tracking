...
int main(int argc, const char *argv[])
{
    ...
    // data location
    string dataPath = "../";
    string filename = "/home/cedric/SFND_3D_Tracking/dat/data.csv";
    // camera
    cv::Mat visImg, vis_3Dimage;
    ...
    ofstream output_stream(filename,std::ios::binary);
    ...
    double detector_t,descriptor_t, match_t;
    /* MAIN LOOP OVER ALL IMAGES */
    std::deque<DataFrame> dataBuffer;
    vector <string> descriptor_type = {"BRIEF","ORB","SIFT"};

    for(string descriptorType : descriptor_type) {
        for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth) {
            ...
            bVis = true;
            if (bVis) {
                show3DObjects((dataBuffer.end() - 1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(2000, 2000),vis_3Dimage, false);
            }
            bVis = false;
            ...
            detector_t = (double) cv::getTickCount();
            ...
            cv::Mat descriptors;
            descriptor_t = (double) cv::getTickCount();
...
//                            cv::waitKey(0);
                    ...
                    if(imgIndex >0)
                    {
                        ...
                        output_stream << detectorType
                                      ...
                                      << "," << std::fixed << std::setprecision(8) << match_t
                                      << "," << std::fixed << std::setprecision(8) << ttcLidar
                                      << "," << std::fixed << std::setprecision(8) << ttcCamera << endl;
                        std::string file_name_3d =
                                "/home/cedric/SFND_3D_Tracking/dat/3D_Objects_Pictures/" + detectorType +
                                descriptorType + to_string(imgIndex) + "frame_with_TTC.png";
                        std::string file_name_ttc_pic =
                                "/home/cedric/SFND_3D_Tracking/dat/TTC_with_Frame_Pictures/" + detectorType +
                                descriptorType + to_string(imgIndex) + "frame_with_TTC.png";
                        cv::imwrite(file_name_3d, vis_3Dimage);
                        cv::imwrite(file_name_ttc_pic, visImg);
                    }
